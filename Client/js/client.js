...
var hac = {
	/**
	 * Polls the server for updated information that is to be displayed in the
	 * UI.
	 * 
	 * @param svc
	 *            Optional parameter. Only poll data for a specific service
	 * @param loaddata
	 *            Optional parameter. If used, svc is mandatory. Before data is
	 *            retrieved, the call will update the data for the service
	 *            (data_load)
	 */
	...
	/**
	 * Inserts or updates an item into the UI. Is called from getData. An update
	 * is only made if the timestamp differs
	 * 
	 * @param data
	 *            Object to be inserted/updated
	 */
	...
	/**
	 * Sorts the objects in the UI based on the priority setting of the infobox
	 * widget
	 */
	sort : function() {
		// Sort
		$('div[id$="_hacsvc"]').sortElements(
				function(a, b) {

					return $("#" + b.id).infobox("option", "priority")
							- $("#" + a.id).infobox("option", "priority");
				});
		// Show all
...
var js = {
	 ...
	 * strings. str = text string as input. E.g. "Hi, my name is ? ?" chr =
	 * character to be replaced. E.g. "?" paramsA = array of strings. E.g.
	 * ["Erik", "Persson"] will return "Hi, my name is Erik Persson"
	 */
	stringChrParams : function(str, chr, paramsA) {
		for ( var i = 0; i < paramsA.length; i++) {
			pos = str.indexOf(chr);
			str = str.substring(0, pos) + paramsA[i]
					+ str.substring(pos + chr.length);
		}
		return str;
	},
...
$(document).ajaxSend(
		function(event, jqxhr, settings) {
			/*
			 * Hide the loading widget for any ajax calls but the cron job and
			 * data load
			 */
...
// Register a ajaxComplete function
...
/*
 * Methods that are to start upon complete loading of the page It will poll for
 * data as well as set the schedulers
 */
$(document).ready(
		function() {
			...
			$("#homepage").on(
					"click",
					function(event) {
						var percent = ($(document).width() - event.pageX)
								/ $(document).width() * 100;
						if (percent < 10)
